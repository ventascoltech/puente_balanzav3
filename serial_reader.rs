...
pub fn start_serial_reader(
    ...
    thread::spawn(move || {
        ...
        loop {
            // ↪ Procesar comandos que llegan desde TCP
            while let Ok(comando) = rx_serial_write.try_recv() {
                if let Err(e) = serial.write_all(&comando).and_then(|_| serial.flush()).with_context(|| {
                    format!(
            ...
            match serial.read(&mut buffer) {
                ...
                Ok(_) => {
                    // No se leyó nada, simplemente continuar
                }
